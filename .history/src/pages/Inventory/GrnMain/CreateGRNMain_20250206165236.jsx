...
const handlePurchaseOrderChange = async (e) => {
  ...
  formik.setFieldValue("purchaseOrderMainId", selectedId);
  ...
  if (selectedId) {
    try {
      ...
      if (response?.data) {
        const supplierIdFromAPI = response.data.supplierId;
        formik.setFieldValue("supplierId", supplierIdFromAPI);
        alert(`Supplier ID set to: ${supplierIdFromAPI}`);
      }
    } catch (error) {
