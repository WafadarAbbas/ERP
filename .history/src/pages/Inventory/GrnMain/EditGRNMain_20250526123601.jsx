...
function EditGRNMain() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response && response.data && response.data.length > 0) {
          ...
          formik.setValues({
            ...
            grnInvoiceNo: GRN?.grnInvoiceNo || "",
    grnInvoiceDate: GRN?.grnInvoiceDate || "",
    branchId: GRN?.branchId || 0,
    branchName: GRN?.branchName || "",
    grnDocNo: GRN?.grnDocNo || "",
    totalGRNVest: GRN?.totalGRNVest || 0,
    totalGRNValue: GRN?.totalGRNValue || 0,
    taxAmount: GRN?.taxAmount || 0,
    furtherTax: GRN?.furtherTax || 0,
    totalTaxCharged: GRN?.totalTaxCharged || 0,
    discount: GRN?.discount || 0,
    totalGRNAmount: GRN?.totalGRNAmount || 0,
    rvMainId: GRN?.rvMainId || 0,
    term: GRN?.term || "",
    narration: GRN?.narration || "",
    supplierNTNNo: GRN?.supplierNTNNo || "",
    supplerName: GRN?.supplerName || "",
    supplierSTRNo: GRN?.supplierSTRNo || "",
    taxChargePlusAmount: GRN?.taxChargePlusAmount || 0,
    fillerStatus: GRN?.fillerStatus || false,
    tax236Amount: GRN?.tax236Amount || 0,
    tax236Rate: GRN?.tax236Rate || 0,
    tax236Status: GRN?.tax236Status || false,
    totalAmountWith236: GRN?.totalAmountWith236 || 0,
    purchaseOrderMainId: GRN?.purchaseOrderMainId || 0,
    supplierId: GRN?.supplierId || 0,
    suppliersSupplerName: GRN?.suppliersSupplerName || "",
          });
        } else {
      ...
      onSubmit: async (values) => {
       ...
       (supplier) => supplier.id === values.supplierId
        ...
        const finalData = {
          ...
          supplerName:selectedsupplierId.name,
          grnInvoiceNo: "string",
          organizationId: 1,
          companyId: 1,
          grnDetails,
        ...
        try {
          ...
          if (response?.status === 200) {
            ...
            throw new Error("Failed to Update the GRN");
          }
        } catch (error) {
          console.error("Error during GRN Update:", error);
          Swal.fire({
            title: "Error",
            text: error.message || "An error occurred while Update the GRN",
            icon: "error",
    ...
    const validationSchemagrn =  Yup.object({
          quantity: Yup.number()
  ...
  validateOnBlur: true,
    ...
    }, [selectedProductVariantId, selectedPurchaseOrder]);
    ...
    const handleEdit = async (id) => {
      try {
        ...
        if (response?.data) {
          const data = response.data[0];  
          console.log( data);
          const indexToEdit = grnDetails.findIndex((item) => item.id === data.id);
          ...
          if (indexToEdit !== -1) {
            ...
            grnFormik.setValues({
              ...
              valueExclGRNTax: data.valueExclGRNTax || 0,
              taxRate: data.taxRate || 0,
              taxAmount: data.taxAmount || 0,
              valueIncGRNTax: data.valueIncGRNTax || 0,
              furtherTax: data.furtherTax || 0,
              furtherTaxAmount: data.furtherTaxAmount || 0,
              product3rdSchedule: data.product3rdSchedule || true,
              itemCode: data.itemCode || '',
              itemName: data.itemName || '',
              fbrProductDesctiptionId: data.fbrProductDesctiptionId || 0,
              fbrProductDesctiptionCode: data.fbrProductDesctiptionCode || '',
              batchId: data.batchId || 0,
              storeId: data.storeId || 0,
              fbrMuId: data.fbrMuId || 0,
              muCode: data.muCode || '',
              taxCharged: data.taxCharged || 0,
              fbrTaxRateId: data.fbrTaxRateId || 0,
              measuringUnitsId: data.measuringUnitsId || 0,
              grnMainId: data.grnMainId || 0,
    ...
    const removeGrnDetail = async (index) => {
      try {
        const detailToDelete = grnDetails[index];
    
        if (!detailToDelete?.id) {
          Swal.fire('Error', 'Invalid item selected to delete', 'error');
          return;
        }
        ...
        // Call delete API
        const response = await ApiCall({
          url: `http://localhost:5022/api/v1/GRNDetail/DeleteGRNDetail/${detailToDelete.id}?organizationId=1&companyId=1`,
          method: 'DELETE',
        });
    
        if (response?.status === 200 || response?.data?.success) {
          // Remove from state
          const newDetails = [...grnDetails];
          newDetails.splice(index, 1);
          setGrnDetails(newDetails);
    
          Swal.fire('Deleted!', 'The detail has been deleted.', 'success');
        } else {
          Swal.fire('Error', 'Failed to delete the detail', 'error');
        }
      } catch (error) {
