  ...
  const addGrnDetail = (detail) => {
    ...
    useEffect(() => {
      if (grnFormik.values.productDefinitionId) {
        alert(`Selected Product Variant ID: ${grnFormik.values.productDefinitionId}`);
      }
...
useEffect(() => {
  if (selectedProductVariantId && selectedPurchaseOrder) {
    const fetchBalanceQuantity = async () => {
      try {
        const response = await ApiCall({
