...
function CreateGRNMain() {
  ...
  const addGrnDetail = (detail) => {
    ...
    const fetchPoBalanceQuantity = async () => {
      try {
        const response = await ApiCall({
          url: "http://localhost:5022/api/v1/PurchseOrderDetails/GetPoBalanceQuantityAndPoQuantity?purchaseOrderMainId=${selectedPurchaseOrder}&productVariantMainId=81,
          method: "GET",
