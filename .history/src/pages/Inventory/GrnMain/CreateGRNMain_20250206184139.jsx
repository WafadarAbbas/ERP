...
function CreateGRNMain() {
  ...
  const [grnInvoiceDate, setgrnInvoiceDate] = useState(null);
  ...
  useEffect(() => {
    if (purchaseOrderDetails && Array.isArray(purchaseOrderDetails) && purchaseOrderDetails.length > 0) {
      ...
      const newgrnInvoiceDate = purchaseOrderDetails[0].poDate;
  
      setSupplierId(newSupplierId);
      setterm(newTerm);
      setgrnInvoiceDate(newgrnInvoiceDate);
      
  
       // Update Formik field value
      formik.setFieldValue("supplierId", newSupplierId);
      formik.setFieldValue("term", newTerm );
      formik.setFieldValue("grnInvoiceDate", newgrnInvoiceDate);
    }
...
{purchaseOrderDetails && Array.isArray(purchaseOrderDetails) && purchaseOrderDetails.length > 0 && (
  ...
  <p>{term !== null ? term : "No term"}</p>
  <p>{grnInvoiceDate !== null ? grnInvoiceDate : "No grnInvoiceDate"}</p>
  </div>
)}
