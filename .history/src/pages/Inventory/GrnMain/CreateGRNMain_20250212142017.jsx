...
function CreateGRNMain() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (selectedPurchaseOrder) { // Check if selectedPurchaseOrder has a value
          const response = await ApiCall({
...
const [poBalanceData, setPoBalanceData] = useState(null);
...
useEffect(() => {
  const fetchPoBalanceData = async () => {
    if (selectedPurchaseOrder && selectedProductVariantId) {
      try {
        const response = await ApiCall({
        ...
        if (response?.data) {
          setPoBalanceData(response.data);
          console.log("PO Balance Data:", response.data);
        } else {
          console.error("Unexpected response format");
        }
      } catch (error) {
        console.error("Error fetching PO balance data:", error);
      }
    }
  };

  fetchPoBalanceData();
}, [selectedPurchaseOrder, selectedProductVariantId]);
