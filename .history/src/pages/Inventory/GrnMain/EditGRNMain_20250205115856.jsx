      ...
      const fetchGRNDetails = async () => {
        try {
          const response = await ApiCall.get(
            `http://localhost:5022/api/v1/GRNDetail/GetByGRNMainId?id=${GRNMainId}&organizationId=1&companyId=1`
          );
          

          if (response.data) {
            setGrnDetails(response.data); // Assuming response.data is an array
          }
        } catch (error) {
