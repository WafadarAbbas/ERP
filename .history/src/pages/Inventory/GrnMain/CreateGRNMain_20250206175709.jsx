...
function CreateGRNMain() {
  ...
  const validationSchema = Yup.object({
    ...
    // For example:
    // purchaseOrderMainId: Yup.number().required('Purchase Order Main is required'),
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        // Make API call to create GRNMain here
        const response = await ApiCall({
          url: "http://localhost:5022/api/v1/GRNMain/Create", // Replace with actual API endpoint
          method: "POST",
          data: values, 
        });
        ...
        if (response.status === 200) {
          ...
          Swal.fire({
            icon: 'success',
            title: 'GRNMain Created Successfully',
            showConfirmButton: false,
            timer: 1500
          });
          navigate("/GRNMain/GRNMain"); // Redirect to GRNMain list
        } else {
          // Handle error (e.g., show error message)
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Failed to create GRNMain.',
          });
        }
      } catch (error) {
        console.error("Error creating GRNMain:", error);
        // Handle error (e.g., show error message)
        Swal.fire({
          icon: 'error',
          title: 'Error',
  ...
  const handlePurchaseOrderChange = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.supplierId) {
        formik.setFieldValue('supplierId', response.data.supplierId);
      }
