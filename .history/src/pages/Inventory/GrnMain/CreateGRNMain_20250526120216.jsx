...
function CreateGRNMain() {
 ...
 const [isLoading, setIsLoading] = useState(false);
  ...
  const validationSchemagrn= (poBalanceQuantity) =>
    const balanceQty = poBalanceQuantity ?? 0; // fallback to 0 if undefined
  ...
  return Yup.object().shape({
    quantity: Yup.number()
      ...
      .test(
        "max-balance",
        `Quantity cannot be more than PO Balance Quantity (${balanceQty})`,
        function (value) {
          // If balanceQty is 0, always fail unless value is also 0
          if (balanceQty === 0) return value === 0;
