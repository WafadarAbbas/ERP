...
const handleRadioChange = (event) => {
  ...
  const fetchPurchaseReturnMain = async () => {
    try {
      ...
      if (response && response.data) {
        ...
        throw new Error('Failed to load PurchaseReturnMain.');
      }
    } catch (error) {
  ...
  const handleDelete = async (PurchaseReturnMainId) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: 'You first need to delete the associated Purchase Detail before deleting the PurchaseReturnMain.',
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response?.status === 200 || response?.status === 204) {
          Swal.fire('Deleted', 'The PurchaseReturnMain has been deleted.', 'success');
          setPurchaseReturnMain(PurchaseReturnMain.filter(PurchaseReturnMain => PurchaseReturnMain.id !== PurchaseReturnMainId));
        } else {
          Swal.fire('Error', 'Failed to delete PurchaseReturnMain', 'error');
        }
      } catch (error) {
        console.error('Error during delete:', error);
        Swal.fire('Error', 'An error occurred while deleting the PurchaseReturnMain', 'error');
      }
    }
  };
    ...
    <div style={{ marginTop: 10 }}>
      <div className='d-flex justify-content-between row'>
        <div className='d-flex flex-column col-sm-7'>
          <h3>PurchaseReturnMain List</h3>
          <h5 style={{ fontWeight: 400 }}>Manage your PurchaseReturnMain</h5>
        </div>
        <div className='col-sm-5' style={{ display: 'flex', gap: '10px', alignItems: 'center', justifyContent: 'flex-end' }}>
          ...
          <Buton  onClick={() => navigate("/PurchaseReturnMain/CreatePurchaseReturnMain")}>Add PurchaseReturnMain</Buton>
        </div>
      </div>
