...
const handleRadioChange = (event) => {
  ...
  const fetchPurchaseReturnMain = async (query = '', fillerStatus = null) => {
    try {
            ...
            <table className="table table-hover">
              <thead className="thead-dark">
                ...
                <th scope="col" style={{ fontSize: 16 }}>
                  ...
                  <th scope="col" style={{ fontSize: 16 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                     Main Invoice                       <button onClick={() => handleSort('PurchaseReturnMainInvoiceNo')} className="btn p-0">
                        {sortOrder === 'asc' && sortColumn === 'PurchaseReturnMainInvoiceNo' ? <FaArrowUp color="green" /> : <FaArrowDown color="red" />}
                      </button>
                    </div>
                  </th>
