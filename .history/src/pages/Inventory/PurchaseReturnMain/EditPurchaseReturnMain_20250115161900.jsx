...
function EditPurchaseReturn() {
  ...
  const fetchpurchaseDetails = async (PurchaseReturnId) => {
    try {
      const response = await ApiCall({
        url: `http://localhost:5022/api/v1/PurchaseReturnDetail/GetByPurchaseReturnMainId?id=${PurchaseReturnId}&organizationId=1&companyId=1`,
        method: "GET",
      });
      ...
      if (response?.data) {
        ...
        const modifiedData = response.data.map(
          ({
            ...
            purchaseReturnMainsPurchaseReturnMainInvoiceNo,
            ...rest
          }) => ({
            ...rest,
            measuringUnitsName: measuringUnitsMeasuringUnitsName ?? "",
            productName: productVariantMainProductName ?? "",
            purchaseReturn: purchaseReturnMainsPurchaseReturnMainInvoiceNo ?? "",
  ...
  useEffect(() => {
    const fetchPurchaseReturn = async () => {
      try {
        const response = await ApiCall({
          url: `http://localhost:5022/api/v1/PurchaseReturnMain/GetyId?id=${PurchaseReturnId}&organizationId=1&companyId=1`,
          method: "GET",
        });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      //     Swal.fire({
      //       title: "Success!",
      //       text: "Purchase Return Main Updated successfully.",
      //       icon: "success",
      ...
      //   } else {
      //     throw new Error("Failed to Update the Purchase Return Main");
      //   }
      // } catch (error) {
      //   Swal.fire({
      //     title: "Error",
      //     text:
      //       error.message || "An error occurred while Updating the Purchase Return Main",
      //     icon: "error",
  ...
  const PurchaseFormik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      const purchaseReturnDetails = {
        ...
        purchaseReturn: selectedPurchaseReturnMain?.name || null,
        organizationId: 1,
        companyId: 1,
  ...
  const fetchProductVariantMains = async () => {
    try {
      ...
      console.error("Error fetching Product Variant Main:", error);
    }
  };
  const fetchPurchaseReturnMain = async () => {
    try {
      ...
      console.error("Error fetching Purchase Return Main:", error);
    }
  };
    ...
    <div style={{ marginTop: 10 }}>
      <div className='d-flex justify-content-between row mb-3'>
        <div className='d-flex flex-column col-sm-7'>
          <h3>Edit Purchase Main</h3>
          <h5 style={{ fontWeight: 400 }}>Manage your Purchase Main Updation</h5>
        </div>
            ...
            <div className="col-md-4 mt-3">
             ...
             <div className="col-md-4 mt-3">
              ...
              <input
                ...
                id="reference"
                name="reference"
                className={`form-control ${formik.touched.reference && formik.errors.reference ? 'is-invalid' : ''
                  }`}
              ...
              {formik.touched.reference && formik.errors.reference && (
                <div className="invalid-feedback">{formik.errors.reference}</div>
              )}
            </div>
