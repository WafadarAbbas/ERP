...
function EditOurchase() {

  const location = useLocation();
  const OurchaseId = location.state?.id;
  const navigate = useNavigate();


  console.log(OurchaseId);



  const fetchpurchaseDetails = async (OurchaseId) => {
    try {
      const response = await ApiCall({
        url: `http://localhost:5022/api/v1/PurchaseDetail/GetByOurchaseId?id=${OurchaseId}&organizationId=1&companyId=1`,
        method: "GET",
      });
  ...
  useEffect(() => {
    if (OurchaseId) {
      fetchpurchaseDetails(OurchaseId);
    }
  }, [OurchaseId]);

  useEffect(() => {
    const fetchOurchase = async () => {
      try {
        const response = await ApiCall({
          url: `http://localhost:5022/api/v1/Ourchase/GetyId?id=${OurchaseId}&organizationId=1&companyId=1`,
          method: "GET",
        });
        ...
        if (response && response.data && response.data.length > 0) {
          ...
          formik.setValues({
            OurchaseInvoiceDate: purchase.OurchaseInvoiceDate || "",
            branchId: purchase.branchId || "",
            branchName: purchase.branchName || "",
            OurchaseDocNo: purchase.OurchaseDocNo || "",
            totalOurchaseVest: purchase.totalOurchaseVest || 0,
            totalOurchaseValue: purchase.totalOurchaseValue || 0,
            taxAmount: purchase.taxAmount || 0,
            furtherTax: purchase.furtherTax || 0,
            totalTaxCharged: purchase.totalTaxCharged || 0,
            discount: purchase.discount || 0,
            totalOurchaseAmount: purchase.totalOurchaseAmount || 0,
  ...
  const formik = useFormik({
    initialValues: {
      ...
      OurchaseDocNo: '',
      totalOurchaseVest: 0,
      totalOurchaseValue: 0,
      taxAmount: 0,
      furtherTax: 0,
      totalTaxCharged: 0,
      discount: 0,
      totalOurchaseAmount: 0,
      rvMainId: 0,
      term: '',
      narration: '',
      supplierNTNNo: '',
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await ApiCall({
          url: "http://localhost:5022/api/v1/Ourchase/UpdateOurchase",
          method: "PUT",
          data: finalData,
        });
  ...
  const PurchaseFormik = useFormik({
    initialValues: {
      ...
      furtherTax: 0,
      furtherTaxAmount: 0,
      valueExclOurchaseTax: 0,
      discount: 0,
      discountAmount: 0,
      taxCharged: 0,
      valueIncOurchaseTax: 0,
      measuringUnitsId: 0,
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      const selectedOurchase = Ourchase.find(
        (Ourchase) => Ourchase.id === values.OurchaseId
      );
      const purchseDetails = {
        ...
        OurchaseInvoiceNo: selectedOurchase?.name || null,
        organizationId: 1,
        companyId: 1,
  ...
  const [Ourchase, setOurchase] = useState([]);
  ...
  const fetchOurchase = async () => {
    try {
      const response = await ApiCall({
        url: "http://localhost:5022/api/v1/Ourchase/GetOurchaseBoxItems?organizationId=1&companyId=1",
        method: "GET",
      });
      setOurchase(response.data);
    } catch (error) {
      console.error("Error fetching Purchase main:", error);
    }
  };
    ...
    <div style={{ marginTop: 10 }}>
      <div className='d-flex justify-content-between row mb-3'>
        ...
        <div className='col-sm-4' style={{ display: 'flex', gap: '10px', alignItems: 'center', justifyContent: 'flex-end' }}>
          <Buton onClick={() => navigate("/Ourchase/Ourchase")}>Move to Purchase Main</Buton>
        </div>
      </div>
      ...
      <div style={{ backgroundColor: 'white', padding: 10, border: "1px solid #d6d4d4" }}>
        ...
        <form onSubmit={PurchaseFormik.handleSubmit}>
          <div className="row border p-2">
            ...
            <div className="col-md-4 mt-3">
              <label htmlFor="valueExclOurchaseTax" className="form-label">Value Excl. Purchase Main Tax</label>
              <input
                type="number"
                id="valueExclOurchaseTax"
                name="valueExclOurchaseTax"
                className={`form-control ${PurchaseFormik.touched.valueExclOurchaseTax && PurchaseFormik.errors.valueExclOurchaseTax ? 'is-invalid' : ''
                  }`}
                value={PurchaseFormik.values.valueExclOurchaseTax}
                onChange={PurchaseFormik.handleChange}
                onBlur={PurchaseFormik.handleBlur}
              />
              {PurchaseFormik.touched.valueExclOurchaseTax && PurchaseFormik.errors.valueExclOurchaseTax && (
                <div className="invalid-feedback">{PurchaseFormik.errors.valueExclOurchaseTax}</div>
              )}
            </div>
            ...
            <div className="col-md-4 mt-3">
              <label htmlFor="valueIncOurchaseTax" className="form-label">Value Including Purchase Main Tax</label>
              <input
                type="number"
                id="valueIncOurchaseTax"
                name="valueIncOurchaseTax"
                className={`form-control ${PurchaseFormik.touched.valueIncOurchaseTax && PurchaseFormik.errors.valueIncOurchaseTax ? 'is-invalid' : ''
                  }`}
                value={PurchaseFormik.values.valueIncOurchaseTax}
                onChange={PurchaseFormik.handleChange}
                onBlur={PurchaseFormik.handleBlur}
              />
              {PurchaseFormik.touched.valueIncOurchaseTax && PurchaseFormik.errors.valueIncOurchaseTax && (
                <div className="invalid-feedback">{PurchaseFormik.errors.valueIncOurchaseTax}</div>
              )}
            </div>
            ...
            <div className="col-md-4 mt-2">
              ...
              <select
                name="OurchaseId"
                className="form-control"
                value={PurchaseFormik.values.OurchaseId}
                onChange={(e) => {
                  PurchaseFormik.setFieldValue(
                    "OurchaseId",
                    Number(e.target.value)
              ...
              >
                ...
                {Array.isArray(Ourchase) &&
                  Ourchase.length > 0 ? (
                  Ourchase.map((Ourchase) => (
                    <option key={Ourchase.id} value={Ourchase.id}>
                      {Ourchase.name}
                    </option>
                  ))
                ) : (
        ...
        <form onSubmit={formik.handleSubmit}>
          <div className="row">
            ...
            <div className="col-md-4">
              <label htmlFor="OurchaseInvoiceDate" className="form-label">
                Invoice Date
              </label>
              <input
                type="datetime-local"
                id="OurchaseInvoiceDate"
                name="OurchaseInvoiceDate"
                className={`form-control ${formik.touched.OurchaseInvoiceDate && formik.errors.OurchaseInvoiceDate ? 'is-invalid' : ''
                  }`}
                value={formik.values.OurchaseInvoiceDate}
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
              />
              {formik.touched.OurchaseInvoiceDate && formik.errors.OurchaseInvoiceDate && (
                <div className="invalid-feedback">{formik.errors.OurchaseInvoiceDate}</div>
              )}
            </div>
            ...
            <div className="col-md-4 mt-3">
              <label htmlFor="OurchaseDocNo" className="form-label">
                Document Number
              </label>
              <input
                type="text"
                id="OurchaseDocNo"
                name="OurchaseDocNo"
                className={`form-control ${formik.touched.OurchaseDocNo && formik.errors.OurchaseDocNo ? 'is-invalid' : ''
                  }`}
                value={formik.values.OurchaseDocNo}
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
              />
              {formik.touched.OurchaseDocNo && formik.errors.OurchaseDocNo && (
                <div className="invalid-feedback">{formik.errors.OurchaseDocNo}</div>
              )}
            </div>

            {/* Total Purchase Main Vest */}
            <div className="col-md-4 mt-3">
              <label htmlFor="totalOurchaseVest" className="form-label">
                Total Vest
              </label>
              <input
                type="number"
                id="totalOurchaseVest"
                name="totalOurchaseVest"
                className={`form-control ${formik.touched.totalOurchaseVest && formik.errors.totalOurchaseVest ? 'is-invalid' : ''
                  }`}
                value={formik.values.totalOurchaseVest}
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
              />
              {formik.touched.totalOurchaseVest && formik.errors.totalOurchaseVest && (
                <div className="invalid-feedback">{formik.errors.totalOurchaseVest}</div>
              )}
            </div>

            {/* Total Purchase Main Value */}
            <div className="col-md-4 mt-3">
              <label htmlFor="totalOurchaseValue" className="form-label">
                Total Value
              </label>
              <input
                type="number"
                id="totalOurchaseValue"
                name="totalOurchaseValue"
                className={`form-control ${formik.touched.totalOurchaseValue && formik.errors.totalOurchaseValue ? 'is-invalid' : ''
                  }`}
                value={formik.values.totalOurchaseValue}
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
              />
              {formik.touched.totalOurchaseValue && formik.errors.totalOurchaseValue && (
                <div className="invalid-feedback">{formik.errors.totalOurchaseValue}</div>
              )}
            </div>
            ...
            <div className="col-md-4 mt-3">
              <label htmlFor="totalOurchaseAmount" className="form-label">
                Total Amount
              </label>
              <input
                type="number"
                id="totalOurchaseAmount"
                name="totalOurchaseAmount"
                className={`form-control ${formik.touched.totalOurchaseAmount && formik.errors.totalOurchaseAmount ? 'is-invalid' : ''
                  }`}
                value={formik.values.totalOurchaseAmount}
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
              />
              {formik.touched.totalOurchaseAmount && formik.errors.totalOurchaseAmount && (
                <div className="invalid-feedback">{formik.errors.totalOurchaseAmount}</div>
              )}
            </div>
