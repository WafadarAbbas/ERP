...
function EditPurchaseMain() {

  const location = useLocation();
  const PurchaseMainId = location.state?.id;
  const navigate = useNavigate();


  console.log(PurchaseMainId);



  const fetchpurchaseDetails = async (PurchaseMainId) => {
    try {
      const response = await ApiCall({
        url: `http://localhost:5022/api/v1/PurchaseDetail/GetByPurchaseMainId?id=${PurchaseMainId}&organizationId=1&companyId=1`,
        method: "GET",
      });
  ...
  useEffect(() => {
    if (PurchaseMainId) {
      fetchpurchaseDetails(PurchaseMainId);
    }
  }, [PurchaseMainId]);
  ...
  useEffect(() => {
    const fetchPurchaseMain = async () => {
      try {
        const response = await ApiCall({
          url: `http://localhost:5022/api/v1/PurchaseMain/GetyId?id=${PurchaseMainId}&organizationId=1&companyId=1`,
          method: "GET",
        });
        ...
        if (response && response.data && response.data.length > 0) {
          ...
          formik.setValues({
            purchaseMainInvoiceDate: purchase.purchaseMainInvoiceDate || "",
            branchId: purchase.branchId || "",
            branchName: purchase.branchName || "",
            purchaseMainDocNo: purchase.purchaseMainDocNo || "",
            totalPurchaseMainVest: purchase.totalPurchaseMainVest || 0,
            totalPurchaseMainValue: purchase.totalPurchaseMainValue || 0,
            taxAmount: purchase.taxAmount || 0,
            furtherTax: purchase.furtherTax || 0,
            totalTaxCharged: purchase.totalTaxCharged || 0,
            discount: purchase.discount || 0,
            totalPurchaseMainAmount: purchase.totalPurchaseMainAmount || 0,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await ApiCall({
          url: "http://localhost:5022/api/v1/PurchaseMain/UpdatePurchaseMain",
          method: "PUT",
          data: finalData,
        });
...
export default EditPurchaseMain;
