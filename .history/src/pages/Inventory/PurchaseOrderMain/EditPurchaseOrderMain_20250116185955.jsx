...
function EditPurchaseOrderMain() {
  ...
  const fetchDetailForEdit = async (id) => {
    try {
      const response = await ApiCall({
        url: `http://localhost:5022/api/v1/PurchseOrderDetails/GetById?id=${id}&organizationId=1&companyId=1`,
        method: "GET",
      });
      ...
      if (response?.data) {
        ...
        formik.setValues({
          ...response.data, // Set form values to the selected detail
        });
      }
    } catch (error) {
            ...
            purchseOrderDetails.map((detail, index) => (
              <tr key={index}>
                ...
                <button
                          ...
                          type='button'
                          onClick={() => fetchDetailForEdit(detail.id)} // Call API on edit click
                        >
                          Edit
                        </button>
                </td>
