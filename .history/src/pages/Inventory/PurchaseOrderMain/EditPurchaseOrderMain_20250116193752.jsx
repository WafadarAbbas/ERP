...
function EditPurchaseOrderMain() {
  ...
  const handleEdit = async (id) => {
    ...
    try {
      const response = await ApiCall({
        url: `http://localhost:5022/api/v1/PurchseOrderDetails/GetyId?id=${id}&organizationId=1&companyId=1`,
        method: "GET",
      });
  
      if (response?.data) {
        // Assuming the response contains the necessary fields to populate the form
        const data = response.data;
  
        // Update the formik values with the fetched data
        PurchaseOrderFormik.setValues({
          poQuantity: data.poQuantity || 2,
