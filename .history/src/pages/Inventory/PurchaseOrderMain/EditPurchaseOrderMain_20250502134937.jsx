...
function EditPurchaseOrderMain() {
  ...
  console.log(POMain);
   ...
   const createRef = useRef(null);
    ...
    const selectedTerms = purchaseOrderTerms.find(t => t.id === values.purchseOrderTermsId);
    const selectedStatus = purchaseOrderStatuses.find(s => s.id === values.purchaseOrderStatusId);
        ...
        const finalData = {
          ...
          companyId: 1,
          id:POMain,
          supplerName: selectedSupplier?.name || "",
          poTerms: selectedTerms?.name || "",
          poStatus: selectedStatus?.name || "",
        };
        ...
        //   }
        // } catch (error) {
        //   console.error("Error during Purchase Order update:", error);
        //   Swal.fire({
        //     title: "Error",
        //     text: error.message || "An error occurred while updating the Purchase Order",
        //     icon: "error",
    ...
    const removepurchseOrderDetails = (index) => {
      const updatedDetails = purchseOrderDetails.filter((_, i) => i !== index);
    ...
    const PurchaseOrderFormik = useFormik({
      ...
      onSubmit: (values, { resetForm }) => {
        ...
        // If editIndex is not null, update the existing item at that index
        if (editIndex !== null) {
          const updatedDetails = [...purchseOrderDetails];
          updatedDetails[editIndex] = purchseOrderDetailsData; // Modify the item at the editIndex
          addpurchseOrderDetails(updatedDetails); // Update the list
      
          setEditIndex(null); // Reset the edit mode
        }
      
        // Set the total number of items in PO
        formik.setFieldValue("poNumberOfItems", purchseOrderDetails.length);
  ...
  const handleEdit = async (id) => {
    try {
      ...
      if (response?.data && Array.isArray(response.data)) {
        ...
        if (data) {
          // Find the index in the local list
          const indexToEdit = purchseOrderDetails.findIndex(
            (item) => item.id === data.id
          );
  
          if (indexToEdit !== -1) {
            setEditIndex(indexToEdit); // Set the edit mode
            ...
            PurchaseOrderFormik.setValues({
              ...
              measuringUnitsName: data.measuringUnitsName || '',
    ...
    try {
      ...
      if (response?.status === 200 && Array.isArray(response.data)) {
        ...
        formik.setValues({
          ...
          poExpectedDeliveryDate: data.poExpectedDeliveryDate || '',
          poReferenceNumber: data.poReferenceNumber || '',
          poDays: data.poDays || '',
          poDueDate: data.poDueDate || '',
          poNumberOfItems: data.poNumberOfItems || 0,
          cashierName: data.cashierName || '',
          isCancel: data.isCancel || false,
          supplierId: data.supplierId || 0,
          purchaseOrderStatusId: data.purchaseOrderStatusId || 0,
          purchseOrderTermsId: data.purchseOrderTermsId || 0,
        });
      } else {
        Swal.fire("Error", "Failed to fetch purchase order", "error");
      }
    } catch (error) {
...
const statusOptions = Array.isArray(purchaseOrderStatuses)
  ? purchaseOrderStatuses.map((s) => ({ value: s.id, label: s.name }))
  : [];

const termsOptions = Array.isArray(purchaseOrderTerms)
  ? purchaseOrderTerms.map((t) => ({ value: t.id, label: t.name }))
  : [];

  const productVariantOptions = Array.isArray(productVariantMainId)
...
const measuringUnitOptions = Array.isArray(measuringUnits)
...
return (
  <div style={{ marginTop: 10 }}>
    <div className='d-flex justify-content-between row mb-4'>
      <div className='d-flex flex-column col-sm-7'>
        <h3>Edit Purchase Order Main</h3>
    ...
    <div style={{ backgroundColor: 'white', padding: 10,marginBottom:10, boxShadow: "0 0 4px 4px rgba(206, 204, 204, 0.5)" }}>
      {/* --------------------------------------- */}
  ...
  <div className="col-10">
    ...
    <Select
      ...
      value={
        supplierOptions.find(opt => opt.value === formik.values.supplierId) || null
      }
      onChange={(selected) =>
        formik.setFieldValue("supplierId", selected ? selected.value : "")
      }
...
<div className="col-md-2 mb-4">
  <label htmlFor="purchaseOrderStatusId" className="form-label">Purchase Order Status</label>
  <Select
    id="purchaseOrderStatusId"
    name="purchaseOrderStatusId"
    options={statusOptions}
    value={
      statusOptions.find(opt => opt.value === formik.values.purchaseOrderStatusId) || null
    }
    onChange={(selected) =>
      formik.setFieldValue("purchaseOrderStatusId", selected ? selected.value : "")
    }
  ...
  />
  {formik.touched.purchaseOrderStatusId && formik.errors.purchaseOrderStatusId && (
    <div className="text-danger">{formik.errors.purchaseOrderStatusId}</div>
  )}
</div>

<div className="col-md-2 mb-4">
  <label htmlFor="purchseOrderTermsId" className="form-label">Purchase Order Terms</label>
  <Select
    id="purchseOrderTermsId"
    name="purchseOrderTermsId"
    options={termsOptions}
    value={
      termsOptions.find(opt => opt.value === formik.values.purchseOrderTermsId) || null
    }
    onChange={(selected) =>
      formik.setFieldValue("purchseOrderTermsId", selected ? selected.value : "")
    }
  ...
  />
  {formik.touched.purchseOrderTermsId && formik.errors.purchseOrderTermsId && (
    <div className="text-danger">{formik.errors.purchseOrderTermsId}</div>
  )}
</div>
  ...
  <Select
    ...
    value={
      productVariantOptions.find(
        (option) => option.value === PurchaseOrderFormik.values.productVariantMainId
      ) || null
    }
    ...
    onBlur={() => PurchaseOrderFormik.setFieldTouched("productVariantMainId", true)}
    isClearable
    className={
...
<div className="col-md-2">
  ...
  <Select
    ...
    value={
      measuringUnitOptions.find(
        (option) => option.value === PurchaseOrderFormik.values.measuringUnitsId
      ) || null
    }
    onChange={(selectedOption) =>
      PurchaseOrderFormik.setFieldValue(
    ...
    onBlur={() => PurchaseOrderFormik.setFieldTouched("measuringUnitsId", true)}
    isClearable
    className={
    ...
    PurchaseOrderFormik.errors.measuringUnitsId && (
      <div className="text-danger">
        {PurchaseOrderFormik.errors.measuringUnitsId}
          ...
          <div>
            <button type="submit" className="btn btn-success mt-3 mb-2 col-1">
              Add Detail
            </button>
          </div>
          <hr></hr>
  ...
  <button
    ...
    onClick={() => handleEdit(detail.purchaseOrderMainId)}
  >
    Edit
  </button>
  <button
