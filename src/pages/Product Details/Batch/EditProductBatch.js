...
const EditProductBatch = (props) => {
  const { selectedProductBatchId } = props;
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await ApiCall({
          url: "http://localhost:5022/api/v1/ProductBatch/UpdateProductBatch",
          method: "PUT",
          data: formData,
        });
        ...
        Swal.fire({
          title: "Error",
          text: error.message || "An error occurred while saving the product Batch",
          icon: "error",
...
useEffect(() => {
  const fetchProductBatch = async () => {
    try {
      const response = await ApiCall({
        url: `http://localhost:5022/api/v1/ProductBatch/GetProductBatchByIdQuery/GetById/${selectedProductBatchId}?organizationId=1&companyId=1`,
        method: "GET",
      });

      if (response && response.data && response.data.length > 0) {
        const productBatch = response.data[0];
        
        
        formik.setValues({
          productBatchNumber: productBatch.productBatchNumber || "",
          productBatchCode: productBatch.productBatchCode || "",
          productBatchDetils: productBatch.productBatchDetils || "",
          productBatchManufacturedDate: productBatch.productBatchManufacturedDate || "",
          productBatchStartDate: productBatch.productBatchStartDate || "",
          productBatchExpirytDate: productBatch.productBatchExpirytDate || "",
          productBatchNoExpiryDate: productBatch.productBatchNoExpiryDate || false,
        });
      } else {
        console.error("Failed to load Product Batch data.");
      }
    } catch (error) {
      console.error("Error fetching Product Batch:", error.message);
    }
  };
  ...
  if (selectedProductBatchId) {
    ...
    <div>
      <button
        ...
        data-bs-target="#EditProductBatchModal"
        ref={props.open}
      >
        Edit ProductBatch
      </button>
      ...
      <div
        ...
        id="EditProductBatchModal"
        tabIndex="-1"
        aria-labelledby="EditProductBatchModalLabel"
        ...
        <div className="modal-dialog modal-lg modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="EditProductBatchModalLabel">
                Edit Product Batch
              </h5>
              <button
                type="button"
