...
const EditHSCode = (props) => {
  const { selectedHSCodeId } = props;

  console.log(selectedHSCodeId);
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await ApiCall({
          url: "http://localhost:5022/api/v1/HSCode/UpdateHSCode",
          method: "PUT",
          data: formData,
        });
        ...
        Swal.fire({
          ...
          text:
            error.message ||
            "An error occurred while saving the product HSCode",
          icon: "error",
  ...
  useEffect(() => {
    const fetchHSCode = async () => {
      try {
        const response = await ApiCall({
          url: `http://localhost:5022/api/v1/HSCode/GetHSCodeByIdQuery/GetById/${selectedHSCodeId}?organizationId=1&companyId=1`,
          method: "GET",
        });

        if (response && response.data && response.data.length > 0) {
          const HSCode = response.data[0];

          formik.setValues({
 
            hsCodeName1: HSCode.hsCodeName1 || "",
            hsCodeName2:  HSCode.hsCodeName2 || "",
            hsCodeName3: HSCode.hsCodeName3 || "",
            hsCodeDescription:  HSCode.hsCodeDescription || "",
          });
        } else {
          console.error("Failed to load Product HSCode data.");
        }
      } catch (error) {
        console.error("Error fetching Product HSCode:", error.message);
      }
    };
  ...
  return (
    <div>
      <button
        ...
        data-bs-target="#EditHSCodeModal"
        ref={props.open}
      >
        Edit HSCode
      </button>
      ...
      <div
        ...
        id="EditHSCodeModal"
        tabIndex="-1"
        aria-labelledby="EditHSCodeModalLabel"
        ...
        <div className="modal-dialog modal-lg modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="EditHSCodeModalLabel">
                Edit Product HSCode
              </h5>
              <button
                type="button"
