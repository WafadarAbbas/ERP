...
function CreatePurchaseMain() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const finalData = {
          ...values,
          purchaseMainInvoiceNo: "string", // change if you get actual value
          organizationId: 1,
          companyId: 1,
          purchseDetails, // make sure this state/variable exists
        };
        ...
        if (response?.data) {
          console.log("Save Purchase Main Response:", response.data);
          Swal.fire({
            icon: "success",
            title: "Saved!",
        ...
        console.error("Error saving Purchase Main:", error);
        Swal.fire({
          icon: "error",
          title: "Error!",
  ...
  const PurchaseFormik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      const selectedMeasuringUnit = measuringUnits.find(
        (unit) => unit.id === values.measuringUnitsId
      );
      const selectedProductVariant = productVariantMains.find(
        (variant) => variant.id === values.productVariantMainId
      );
      const selectedPurchaseMainItem = PurchaseMain.find(
        (item) => item.id === values.purchaseMainId
      );

      const detail = {
        ...values,
        measuringUnitsName: selectedMeasuringUnit?.name || '',
        productName: selectedProductVariant?.name || '',
        purchaseMainInvoiceNo: selectedPurchaseMainItem?.name || '',
        organizationId: 1,
        companyId: 1,
      };

      addpurchseDetails(detail);
  ...
  // ---------------------- Live Calculation ----------------------
  useEffect(() => {
    const amount = PurchaseFormik.values.quantity * PurchaseFormik.values.productPrice;
    const discountAmount = (amount * PurchaseFormik.values.discount) / 100;
    const valueExclPurchaseMainTax = amount - discountAmount;
    const taxAmount = (valueExclPurchaseMainTax * PurchaseFormik.values.taxRate) / 100;
    const valueIncPurchaseMainTax = valueExclPurchaseMainTax + taxAmount;

    PurchaseFormik.setFieldValue("amount", amount, false);
    PurchaseFormik.setFieldValue("discountAmount", discountAmount, false);
    PurchaseFormik.setFieldValue("valueExclPurchaseMainTax", valueExclPurchaseMainTax, false);
    PurchaseFormik.setFieldValue("taxAmount", taxAmount, false);
    PurchaseFormik.setFieldValue("valueIncPurchaseMainTax", valueIncPurchaseMainTax, false);
  }, [
    PurchaseFormik.values.quantity,
    PurchaseFormik.values.discount,
    PurchaseFormik.values.productPrice,
    PurchaseFormik.values.taxRate,
  ]);

  // ---------------------- Helper Functions ----------------------
  const addpurchseDetails = (detail) => setpurchseDetails([...purchseDetails, detail]);
  const removepurchseDetails = (index) =>
    setpurchseDetails(purchseDetails.filter((_, i) => i !== index));

const handleGRNChange = async (e) => {
  const selectedId = Number(e.target.value);
  formik.setFieldValue("grnMainId", selectedId);

  if (!selectedId) return;

  try {
    // Sirf GRNDetail ke liye call yahan
    const detailResponse = await ApiCall({
      url: `http://localhost:5022/api/v1/GRNDetail/GetByGRNMainId?id=${selectedId}&organizationId=1&companyId=1`,
      method: "GET",
    });

    if (detailResponse?.data) {
      setGrnDetails(detailResponse.data);
      console.log("GRN Details Response:", detailResponse.data);
    }
  } catch (error) {
    console.error("Error fetching GRNDetail data:", error);
  }
};
useEffect(() => {
  const fetchGRNMain = async () => {
    if (!formik.values.grnMainId) return;

    try {
      const response = await ApiCall({
        url: `http://localhost:5022/api/v1/GRNMain/GetyId?id=${formik.values.grnMainId}&organizationId=1&companyId=1`,
        method: "GET",
      });

      if (response?.data?.length > 0) {
        const grnData = response.data[0];
        formik.setValues({
          ...formik.values,
          taxAmount: grnData.taxAmount,
          furtherTax: grnData.furtherTax,
          totalTaxCharged: grnData.totalTaxCharged,
          discount: grnData.discount,
          rvMainId: grnData.rvMainId,
          term: grnData.term,
          narration: grnData.narration,
          supplierNTNNo: grnData.supplierNTNNo,
          supplierSTRNo: grnData.supplierSTRNo,
          taxChargePlusAmount: grnData.taxChargePlusAmount,
          fillerStatus: grnData.fillerStatus,
          tax236Amount: grnData.tax236Amount,
          tax236Rate: grnData.tax236Rate,
          tax236Status: grnData.tax236Status,
          totalAmountWith236: grnData.totalAmountWith236,
          supplierId: grnData.supplierId,
        });
      }
    } catch (error) {
      console.error("Error fetching GRNMain data:", error);
    }
  };

  fetchGRNMain();
}, [formik.values.grnMainId]);
  // ---------------------- Fetch Data on Mount ----------------------
  ...
  const fetchGRNMain = async () => {
    try {
      ...
      if (response?.data) setGRNMain(response.data);
    } catch (error) {
  ...
  const fetchMeasuringUnits = async () => {
    try {
      ...
      if (response?.data) setMeasuringUnits(response.data);
    } catch (error) {
  ...
  const fetchProductVariantMains = async () => {
    try {
      ...
      if (response?.data) setProductVariantMains(response.data);
    } catch (error) {
  ...
  const fetchPurchaseMain = async () => {
    try {
      ...
      if (response?.data) setPurchaseMain(response.data);
    } catch (error) {
          ...
          <div className="row">
            <div className="col-md-2 mt-3">
              ...
              <select
                ...
                onChange={(e) => {
                  formik.setFieldValue("grnMainId", Number(e.target.value));
                  handleGRNChange(e);
                }}
              ...
              </select>              {formik.touched.grnMainId &&
          ...
          <div className="row border p-2 mt-2" style={{ boxShadow: "0 0 4px 4px rgba(206, 204, 204, 0.5)" }}>
            ...
            <div className="col-md-2 mt-3">
              ...
              <select
                ...
                className={`form-control ${PurchaseFormik.touched.productVariantMainId && PurchaseFormik.errors.productVariantMainId
                    ? "is-invalid"
                    : ""
                  }`}
              ...
              {PurchaseFormik.touched.productVariantMainId && PurchaseFormik.errors.productVariantMainId && (
                <div className="invalid-feedback">{PurchaseFormik.errors.productVariantMainId}</div>
              )}
            </div>
            ...
            <div className="col-md-2 mt-3">
              <label htmlFor="valueExclPurchaseMainTax" className="form-label">Vest Purchase Main Tax</label>
              <input
                type="number"
