...
import { FaFilter, FaEdit, FaEye } from 'react-icons/fa'; // Importing edit and view icons
import { FaArrowUp, FaArrowDown } from 'react-icons/fa'; // Importing sort icons
...
function ExpireProducts() {
  ...
  const sortedExpireProductss = [...ExpireProductss].sort((a, b) => {
    if (sortOrder === 'asc') {
    ...
    <div style={{ marginTop: 10 }}>
      <div className='d-flex justify-content-between row'>
        <div className='d-flex flex-column col-sm-7'>
          <h3 style={{ color: '#092c4c' }}>ExpireProducts List</h3>
          <h5 style={{ fontWeight: 400, color: '#092c4c' }}>ExpireProducts List</h5>
        </div>
        <div className='col-sm-5' style={{ display: 'flex', gap: '10px', alignItems: 'center', justifyContent: 'flex-end' }}>
          <i className="fa fa-print text-primary fs-2 me-2 p-2 bg-white border border-grey rounded-3 cursor-pointer" onClick={handleClick} ></i>
          <i className="fa-solid fa-file-pdf text-danger fs-2 me-2 p-2 bg-white border border-grey rounded-3 cursor-pointer " onClick={handleClick} ></i>
          <i className="fa fa-file-excel-o fs-2 me-2  p-2 bg-white border border-grey rounded-3 cursor-pointer" onClick={handleClick} style={{ color: 'green' }}></i>
        </div>
      </div>
      ...
      <div className="d-flex justify-content-between align-items-center">
        ...
        <div className="table-responsive">
          <table className="table table-hover">
            ...
            <tbody>
              {sortedExpireProductss.map((product, index) => (
                <tr key={index}>
