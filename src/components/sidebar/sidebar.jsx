...
function Sidebar() {
		...
		var handleSidebarMenuToggle = function(menus, expandTime) {
			menus.map(function(menu) {
				menu.onclick = function(e) {
					...
					if (targetItemElm.classList.contains('expand') || (targetItemElm.classList.contains(' ') && !target.style.display)) {
					...
					elm.onmouseenter = function() {
						...
						if (appElm && appElm.classList.contains('app-sidebar-minified')) {
							...
							if (targetMenuHtml) {
								...
								var lightMode      = (sidebar.getAttribute('data-bs-theme') === 'light') ? true : false;
								var windowHeight  = document.body.clientHeight;
	...
	return (
		...
		{({toggleAppSidebarMinify, toggleAppSidebarMobile, appSidebarTransparent, appSidebarGrid, appSidebarLight}) => (
			<React.Fragment>
				<div id="sidebar" className={'app-sidebar ' + (appSidebarTransparent ? 'app-sidebar-transparent' : '') + (appSidebarGrid ? 'app-sidebar-grid' : '')} data-bs-theme={appSidebarLight ? '' : 'light'} style={{ border: '2px solid #ddd' }}>
				...
				<div className="app-sidebar-bg" data-bs-theme={appSidebarLight ? '' : 'light'}></div>
